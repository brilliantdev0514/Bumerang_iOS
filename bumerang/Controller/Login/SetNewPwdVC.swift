...
class SetNewPwdVC: BaseViewController {
    ...
    func callSetNewPwdAPI(_ newPwd : String) {
        ...
        // call api
        UserApiManager.changePWD(userId: self.userId, pwd: newPwd) { (isSuccess, data) in
            
            self.hideALLoadingView()
            if isSuccess { self.hideMe() }
            else {
                if data == nil {
                    if self.countCallApi == 2 {
                        self.countCallApi += 1
                        self.callSetNewPwdAPI(newPwd)
                    } else { self.showToast(R_EN.string.CONNECT_FAIL)
                    }
                } else {
